...
def draw_histogram():
    ...
    plt.hist([unmod_df["SA"],mod_df["SA_mod"], mod_df["SA_same_mod_with_mod"],rand_df], color=[low, medium, high, a], ec='black', bins=15)
    plt.xlabel('SA')
    plt.ylabel('intensity')
    # plt.axis("scaled")
    handles = [Rectangle((0,0),1,1,color=c,ec="k") for c in [low,medium,high, a]]
    labels= ["unmod-unmod","unmod-mod", "mod-mod", "random"]
